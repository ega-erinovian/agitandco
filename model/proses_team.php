    ...
    switch($_POST['kelola']){
        ...
        case 'edit':
            ...
            if(empty($string_img) == 0){
                // $query = "UPDATE ".$tableName." SET `id_team`='$id_team', `name`='$nama', `deskripsi`='$deskripsi', `idyoutube`='$idyoutube', `devisi`='$devisi', `img`='$string_img' WHERE `id_team` = '$id_team'";
                $query = "UPDATE " . $tableName . " SET ";
                $setClauses = [];

                if (!empty($id_team)) {
                    $setClauses[] = "`id_team` = '$id_team'";
                }
                ...
                if (!empty($devisi)) {
                    $setClauses[] = "`devisi` = '$devisi'";
                }

                if (isset($delete_img)) {
                    $originalArray = explode(',', $tmp_img_name);
                    foreach ($delete_img as $img) {
                        $path = realpath('../assets/img/projects/' . $id_team . '/' . $img);
                        unlink($path);
